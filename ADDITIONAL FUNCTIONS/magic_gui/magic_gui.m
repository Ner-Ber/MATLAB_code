...
% Last Modified by GUIDE v2.5 13-Apr-2015 13:15:08
...
% --- My Startup Code --------------------------------------------------
    ...
    imshow(imgOriginal, 'InitialMagnification', 'fit', 'parent', handles.axesImage);
...
% 		return;
	...
	if get(handles.chkAutoAnalyze,'Value')
		AnalyzeSingleImage(handles, imgOriginal);
	end
...
function UpdateAnalyzeButtonCaption(handles)
	...
	if length(Selected) > 1 
        buttonCaption = {'Step 5:  Analyze '};   % MATLAB quirk - needs to be cell array to keep trailing spaces.
	...
	elseif length(Selected) == 1 
        set(handles.btnAnalyze, 'string', 'Step 5:  Analyze 1 image');
		set(handles.btnAnalyze, 'Tooltipstring', 'Display and analyze the selected image');		
	else
        set(handles.btnAnalyze, 'string', 'Step 5:  Analyze no images');
...
%     resultsArray = zeros(numberOfSelectedFiles, 6);
	...
	for j = 1 : numberOfSelectedFiles    % Loop though all selected indexes.
		...
		% Analyze this image.
        resultsArray = AnalyzeSingleImage(handles, imgOriginal);
...
function [resultsArray] = AnalyzeSingleImage(handles, theImage)
	try
		resultsArray = zeros(1, 3);	% Initialize to make sure that something will always be returned.

		% Put your custom code here...
		theMean = mean2(theImage(:));	% Just an example.
...
% I'm only saving the image folder and "Send to Excel" settings here, but you could 
% save all the settings of all the checkboxes, scrollbars, etc. on the GUI, like
...
% Then add an argument 'guiSettings' to the input arguments of save() function.
%       save('Magic_GUI.mat', 'lastUsedImageFolder', 'guiSettings');
...
try
	% Load up the initial values from the mat file.
	matFullFileName = fullfile(handles.macroFolder, 'Magic_GUI.mat');
