function pre=anls_pre_conditions(exp,event,Tstart,lineNum,smtXsg)
%event - Vector
...
for k=1:length(event)
    ...
    e=acq132_event_get_data(exp,event(k),Tstart-0.5,Tstart+0.5,1,'x_sg','F','N','Uxy','Uxx','Uyy','Sxy','Sxx','Syy');
    %e=smooth_sg_x(e,smtXsg); % could be done smarter and faster
    phE=phantomGetLines(exp,event(k),Tstart-0.5,'min',Tstart+0.1,1000,1,'',lineNum);
    e.lines=phE.lines;
    e.x=phE.x;
