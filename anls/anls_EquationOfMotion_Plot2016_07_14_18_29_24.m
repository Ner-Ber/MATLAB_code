function anls_EquationOfMotion_Plot2016_07_14_18_29_24(anl)

%Lubricated case, Slow events - 16,18,24 
...
%---choose strain gages that will affect Uxy
%index_sg=(6:length(anl.y3_5.x_sg)); %Slow events 16,18,24
index_sg=(1:length(anl.y3_5.x_sg)); %Fast events 

[~,index]=min(abs(anl.y3_5.x_sg-57));
index_sg_Plot=(index:length(anl.y3_5.x_sg)); %choose strain gages for plot
...
%---For 200mm
anl.y3_5.Gamma(anl.y3_5.x_sg<100)=Gamma/1.67;%1.3;
anl.y3_5.Gamma(anl.y3_5.x_sg>100)=Gamma;
anl.y3_5.Gamma(anl.y3_5.x_sg>135)=Gamma/1.67;
...
for j=1:length(anl.e)
    ...
    x=[0;0.01; anl.y3_5.x_sg(index_sg)']*1e-3;
    ...
    for l=1:length(anl.y3_5.x_sg)
        [~, index]=min(abs(anl.frontRaw{j}.xv - anl.y3_5.x_sg(l) ));
        if (index<4)
            anl.y3_5.Cf(j,l)=0;
        else
            anl.y3_5.Cf(j,l)=mean( anl.frontRaw{j}.v(index-3:index+3) );
        end
    end
...
for j=1:length(anl.e)
    ...
    plot(anl.frontRaw{j}.xv(index),smooth(anl.frontRaw{j}.v(index),3),'.-')
    subplot(1,2,2);
    plot(anl.frontRaw{j}.Gs_Gamma(index),smooth(anl.frontRaw{j}.v(index),3),'.-');
...
my_legend_add(num2str(anl.e') );legend off
plot(eqm{j}.g(Cf_tmp).^-1,Cf_tmp,'black.-')
title(anl.lgnd{1}(1:end-3));
...
for j=1:length(anl.e)
    ...
    %plot(anl.y3_5.x_sg(index_sg_Plot),1e-3*anl.y3_5.UxxP(j,index_sg_Plot),'o-');
    ...
    %plot(anl.y3_5.x_sg(index_sg_Plot),1e-3*anl.y3_5.UyyP(j,index_sg_Plot),'o-');
    my_legend_add( num2str(anl.e(j)) );
    
    % % --------plot Uii*cf Vs G/G_s
    figure(figN7);
    subplot(1,2,1);
    ...
    subplot(1,2,1);
    plot(anl.y3_5.Cf(j,index_sg_Plot),anl.y3_5.UxxP(j,index_sg_Plot),'o');
    my_legend_add( num2str(anl.e(j)));
    
    subplot(1,2,2);
    plot(anl.y3_5.Cf(j,index_sg_Plot),anl.y3_5.UyyP(j,index_sg_Plot),'o');
    my_legend_add( num2str(anl.e(j)));
...
% %-------------plot Uii Vs Cf  &  Uii*cf Vs Gs for each sg (all sg)
...
for j=1:length(anl.y3_5.x_sg)
    ...
    figure(figN4);
    subplot(1,2,1);
    plot(anl.y3_5.Cf(:,j),anl.y3_5.UxxP(:,j),'o');
    hold all;
    my_legend_add( num2str(anl.y3_5.x_sg(j)) );
    xlabel('Cf');
    ylabel('Uxx');
    title(anl.lgnd{1}(1:end-3));
    xlim([0 Cr]);
    ...
    subplot(1,2,2);
    plot(anl.y3_5.Cf(:,j),anl.y3_5.UyyP(:,j),'o');
    hold all;
    my_legend_add( num2str(anl.y3_5.x_sg(j)) );
    xlabel('Cf');
    ylabel('Uyy');
    ...
    figure(figN5);
    subplot(1,2,1);
    x=anl.y3_5.Gs(:,j)./anl.y3_5.Gamma(j);
    y=1e-3*anl.y3_5.UxxP(:,j).*anl.y3_5.Cf(:,j)./anl.y3_5.Gamma(j).^0.5;
    semilogy(x,y,'o');
    ...
    subplot(1,2,2);
    x=anl.y3_5.Gs(:,j)./anl.y3_5.Gamma(j);
    y=1e-3*anl.y3_5.UyyP(:,j).*anl.y3_5.Cf(:,j)./anl.y3_5.Gamma(j).^0.5;
    semilogy(x,y,'o');
    hold all;
    my_legend_add( num2str(anl.y3_5.x_sg(j)) );
    xlabel(' Gs/Gamma');
...
figure(figN4);
subplot(1,2,1);
G_ratio=sol.Gamma/max(anl.y3_5.Gamma);
