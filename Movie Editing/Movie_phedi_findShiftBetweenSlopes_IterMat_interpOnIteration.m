...
% [chosenPeakLoc1, side1]= Movie_phedi_find_relevant_peak(interpedSig_1, PhediCoordinates_interp);
% [chosenPeakLoc2, side2] = Movie_phedi_find_relevant_peak(interpedSig_2, PhediCoordinates_interp);
...
% side = side2;
% side(~(side1==side2)) = 0;
side = zeros(size(PhediCoordinates));
...
for phedi_idx = 1:length(PhediCoordinates)
    ...
    %-- the false peak are ment just for fincding the right phedi-envoirment
    currentPkLoc1_false = chosenPeakLoc1(phedi_idx);
    currentPkLoc2_false = chosenPeakLoc2(phedi_idx);
...
%     currentPkLoc2 = chosenPeakLoc2(phedi_idx);
    ...
    %% crop signals to phedi envoirment
    Dist4Crop = 5*abs(current_phedi-currentPkLoc1_false);   % multiply by 5 to get a descent dstace from the slope since the interpolation is boudary condition dependant
    cropRegion = current_phedi-Dist4Crop:current_phedi+Dist4Crop;
    sig1Crop = signal1(cropRegion);
    sig2Crop = signal2(cropRegion);
    
    %% interp the relevant region
    x_interp_relevant = x_interp(min(cropRegion)<=x_interp & max(cropRegion)>=x_interp);
    
    interpedSig_1 = interp1(x_measure,signal1,x_interp_relevant,interpMethod);
    interpedSig_2 = interp1(x_measure,signal2,x_interp_relevant,interpMethod);
    
    %% Normalize the signals:
    %-- find sides and peaks:
    phedi_newCoor = find(current_phedi==x_interp_relevant);
    [chosenPeakLoc1, side1] = Movie_phedi_find_relevant_peak(interpedSig_1, phedi_newCoor);
    [chosenPeakLoc2, side2] = Movie_phedi_find_relevant_peak(interpedSig_2, phedi_newCoor);
    %-- write sides:
    side_i = side2;
    side_i(~(side1==side2)) = 0;
    side(phedi_idx) = side_i;
    
    stretching_factor = interpedSig_1(chosenPeakLoc1)/interpedSig_2(chosenPeakLoc2);
...
%     HorzDist2Peak = abs(current_phedi-currentPkLoc1);
    HorzDist2Peak = abs(phedi_newCoor - chosenPeakLoc1);
    original_slope_location = phedi_newCoor-HorzDist2Peak:phedi_newCoor+HorzDist2Peak;
    ...
    shiftsMat(shiftsMat>length(interpedSig_2)) = length(interpedSig_2);
    %-- create matrix of signal 2 shofted in various ways
    interpedSig_2_shifted = interpedSig_2(shiftsMat);
    slope2_mins = bsxfun(@minus,interpedSig_2_shifted,min(interpedSig_2_shifted,[],2));
    slope2_strchd = bsxfun(@rdivide,slope2_mins,max(slope2_mins,[],2));
    slope1_mins = interpedSig_1(original_slope_location)-min(interpedSig_1(original_slope_location));
