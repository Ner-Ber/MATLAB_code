function [phediOfSlopeShift, side, stretching_factor_vec] = Movie_phedi_findShiftBetweenSlopes_UseIntrpStruct(SigStruct1, SigStruct2, PhediCoordinates,varargin)
    ...
    [~,PhediCoordinates_interp] = min(abs(bsxfun(@minus,x_evalPeaks(:),PhediCoordinates(:)')),[],1);
    
    %-- find approximate peak using otiginal signal:
    [originalPeakLoc1, side1] = Movie_phedi_find_relevant_peak(SigStruct1.y, round(PhediCoordinates));
    [originalPeakLoc2, side2] = Movie_phedi_find_relevant_peak(SigStruct2.y, round(PhediCoordinates));
    
    %-- find peaks of interpolated curve:
    [interped_pks1,interped_locs1] = findpeaks(signal1,x_evalPeaks);
    [interped_pks2,interped_locs2] = findpeaks(signal2,x_evalPeaks);
    
    %--- find closest interpolated peak to original one
    %-- signal1:
    x_diff = bsxfun(@minus,originalPeakLoc1(:),interped_locs1(:)');
    y_diff = bsxfun(@minus,SigStruct1.y(originalPeakLoc1(:)),interped_pks1(:)');
    Eucld_dist = sqrt(x_diff.^2 + y_diff.^2);
    [~,I1] = min(Eucld_dist,[],2);
    %-- signal2:
    x_diff = bsxfun(@minus,originalPeakLoc2(:),interped_locs2(:)');
    y_diff = bsxfun(@minus,SigStruct2.y(originalPeakLoc2(:)),interped_pks2(:)');
    Eucld_dist = sqrt(x_diff.^2 + y_diff.^2);
    [~,I2] = min(Eucld_dist,[],2);
    %-- transform to new interpolated coordinates:
    [~,interped_locs1] = findpeaks(signal1);
    [~,interped_locs2] = findpeaks(signal2);
