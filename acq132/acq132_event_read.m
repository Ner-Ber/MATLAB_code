...
fid= fopen(file,'r');
t=fread(fid, 'float64');
t_trig=t(pre-1);%+(t(pre-1)-t(pre-2))/2;%[sec] Strange!! but was checked. would expect t_trig=t(pre)+(t(pre+1)-t(pre))/2;
%t_trig=t(pre-1)+(t(pre-1)-t(pre-2))/2;%[sec] Strange!! but was checked. would expect t_trig=t(pre)+(t(pre+1)-t(pre))/2;
%If the trigger channel is measure more accurate trigger time is calculated in acq132_event_get_data
