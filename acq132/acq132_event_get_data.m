...
for k=1:length(event_vec)
    ...
    t_trig=t_trig+exp_details.triggerDelay; %[sec]
    ...
    acq132=acq132_convert_raw_to_data_ChooseBoundary(exp_dir,ch_Master,ch_Slave);  %- Defaults is PlaneFlag = 'PlaneStrain'
    
    % -- If 'trigger' was measured better syncronization is possible
    if (isfield(acq132,'trigger'))
       [~, index]=min(abs(t));
        dt=mean(diff(t));
        dt_trigger=dt*( acq132.trigger(index) -acq132.trigger(end) ) / (acq132.trigger(1) -acq132.trigger(end))-0.5*dt;
    else
        dt_trigger=0;
    end

    acq132.t=t-dt_trigger;
    acq132.t_trig=t_trig-dt_trigger;
