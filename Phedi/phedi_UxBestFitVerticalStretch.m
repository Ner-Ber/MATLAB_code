%[residualsCell, RMS_vec, shifts_vec] = phedi_UxBestFitVerticalStretch(PhediStruct, varargin)
%varargin: [reduceSin, plotFits, plotInBlock, XLim, Handle]
% defaults: (1,0,1,[-0.05 -0.01])
%
% phedi_UxBestFitVerticalShift will calculate the vertical stratch needed for a good
% fit between LEFM prediction and the Ux measurement.
%


function [residualsCell, RMS_vec, stretch_vec] = phedi_UxBestFitVerticalStretch(PhediStruct, varargin)
    ...
    stretch_vec = zeros(size(PhediLocReduced,2),1);
...
%         stretchVec = -peak2peak(Y_model):1e-7:peak2peak(Y_model);
        stretchVec = linspace(0.1,10,1e3);
        StretchedMat = bsxfun(@times,Y_model(:),stretchVec);
        DiffMat = bsxfun(@minus,StretchedMat,YY(:));
        [minRMS,I] = min(rms(DiffMat,1));
        minStretch = stretchVec(I);
        residualsCell{i} = DiffMat(:,I);
        if isempty(minRMS); minRMS=nan; end;
        if isempty(minStretch); minStretch=nan; end;
        RMS_vec(i) = minRMS;
        stretch_vec(i) = minStretch;
        
        if plotFits
            plot(spaceAxis(:,i),PhediLocReduced(:,i),...
                'Marker','s','MarkerSize',3,'MarkerFaceColor',[0.6 0.6 0.6],...
                'LineWidth',1,'Color',Colors(i,:));
            plot(PhediStruct.solAtInter.x,PhediStruct.solAtInter.Ux*minStretch,'-','Color',Colors(i,:),'LineWidth',2);
        end
    end
