...
subplot(2,2,1);
plot(spatialVec, RowOverTime_preDamp(firstFrame4Phedi,:),'*','MarkerSize',6);
hold on;
% plot((0:(size(RowOverTime_true,2)-1))/res_true, RowOverTime_true(firstFrame4Phedi,:));
plot((x_true-1)/res, RowOverTime_true(firstFrame4Phedi,:));
plot(spatialVec(phedisInitialLocsInPix),RowOverTime_preDamp(firstFrame4Phedi,phedisInitialLocsInPix),'ro');

x_sig = 1:frameLength;
x_evalIntrp = 1:10^-numOOM:frameLength;
signal1_intrpd = csaps(x_sig,RowOverTime_preDamp(firstFrame4Phedi,:),interpP,x_evalIntrp);
plot((x_evalIntrp-1)/res,signal1_intrpd,'g')
plot(spatialVec, RowOverTime(firstFrame4Phedi,:),'*','MarkerSize',6);
hold off;
xlim([-1 1]*2e-4+mean(spatialVec(phedisInitialLocsInPix(1:2))))
ylim auto

%% plot heat map with trajectories
subplot(2,2,2); hold on;
shiftByLocation = mean(spatialVec);
xImagesc = [spatialVec(1) spatialVec(end)];
yImagesc = [t_vec_s(1), t_vec_s(end)];
imagesc(xImagesc,yImagesc,RowOverTime);

%--- plot it:
for i = phdIndx(:)'
%     plot(PhediLocation(:,i)+measuredPhedisFromPlot(i),...
    plot(PhediLocation(:,i),...
        timeVec,...
...
plot(center_of_mass/res,t_vec_s);

%% plot original translation and phedi location
subplot(2,2,3);
hold on;
%-- original shift
plot(t_vec_s(firstFrame4Phedi:lastFrame4Phedi),shiftVector(firstFrame4Phedi:lastFrame4Phedi)/res_true,'.-');

PhediLocReduced = bsxfun(@minus,PhediLocation,mean(PhediLocation(1:30,:),'omitnan'));
%--- plot it:
for i = phdIndx(:)'
    plot(timeVec,PhediLocReduced(:,i),...
        FigMarks{i},'Color',FigColors(i,:),'DisplayName',LGND{i});
end
...
hold on;
plot(t_vec_s,center_of_mass_zero/res,'LineWidth',1.5);
...
%% plot difference between phedis and original slope

subplot(2,2,4); hold on;
PhediLocReduced = bsxfun(@minus,PhediLocation,mean(PhediLocation(1:30,:),'omitnan'));
phediDiff = bsxfun(@minus,PhediLocReduced, shiftVector(firstFrame4Phedi:lastFrame4Phedi)'/res_true);
%--- plot it:
for i = phdIndx(:)'
    plot(timeVec,phediDiff(:,i),...
        FigMarks{i},'Color',FigColors(i,:),'DisplayName',LGND{i});
end
...
hold off;
%%
suptitle({['Vslip=',num2str(Vslip),'   minimal RMS method'],...
    ['   N-OOM=',num2str(numOOM),'   interfrnce dist.=',num2str(Y)]});