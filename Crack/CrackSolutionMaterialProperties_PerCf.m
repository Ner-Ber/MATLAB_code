function [Cd, Cs, Cr, nu, ro, E, mu, Gamma, PlaneStrain, tau_p, Xc0]=CrackSolutionMaterialProperties_PerCf(Cf, varargin)
% [Cd, Cs, Cr, nu, ro, E, mu, Gamma, PlaneStrain, tau_p, Xc0]=CrackSolutionMaterialProperties_lowFreq(PlaneFlag)
% based on data from Read & Duncan 1981 for approx. 1Hz.


PlaneFlag = setDefaults4function(varargin,'PlaneStrain');

freq = Cf/3e-3;     % based on a typical length (3mm, measured from slip velocity on interface) and Cf

% % %---PMMA

%-- get E youngs modulus
EvsFreq = empModel_EvsFreq();
E = ppval(EvsFreq,log10(freq))*1e9;
%-- get nu poisson's ratio
NuvsFreq = empModel_NuVsFreq();
nu = ppval(NuvsFreq,log10(freq));

ro=1.17E3;%[Kg/m^3];
Cd = sqrt(E*(1-nu)/((1+nu)*(1-2*nu)*ro));
Cs = sqrt((E)/(2*ro*(1+nu)));
mu=Cs^2*ro;
