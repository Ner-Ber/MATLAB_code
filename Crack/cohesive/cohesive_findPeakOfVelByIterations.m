function [X_peak, Y_peak] = cohesive_findPeakOfVelByIterations(v,h,Gamma,variableName1,variableVal1,varargin)
    ...
    [cohesiveFun,PlaneFlag,convergeRes] = setDefaults4function(varargin,'exp','PlaneStrain',5e-5);
    ...
    while DX>=convergeRes
        ...
        [pks,locs] = findpeaks(currentCohesiveStruct.vx);
...
%         negativePksLogic = pks>0;
        [~,I] = max(pks);
        newRegion = sort(currentCohesiveStruct.x([-1 +1]+locs(I)));
        X_vec = linspace(newRegion(1),newRegion(2),Nx);
    end
    X_peak = currentCohesiveStruct.x(locs(I));
    Y_peak = pks(I);
