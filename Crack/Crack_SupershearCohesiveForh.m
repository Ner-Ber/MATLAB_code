function sol=Crack_SupershearCohesiveForh(g_in,y)
...
%------Define Gamma and tau_s
 Gamma=1; %[J/m^2]
 tau_s=1E6;%tau_s=mu/1000;%maximal shear stress
% 
% %David's simulation
% %Gamma=3.4427;
% %tau_s=1.5792e6;
...
xStart=-35E-3;%[m]
xEnd=35E-3;%[m]
x=round(xStart/Xc):dx:round(xEnd/Xc);%[Xc] distance from the crack tip ,theta=0;
...
% %h=@(w) 1/2*( I(w)- I(2*z-w));
% l=min(z--1,0-z);
% Io=integral(v,z-l/2,z+l/2);%eq.6.9.118
% Io=Io+integral(I,-1,z-l/2)+integral(I,z+l/2,0,'RelTol',1e-3,'AbsTol',1e-5)
