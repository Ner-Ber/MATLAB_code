function [x_min_Ttip, relevantLocations] = signal_ChangeTime2Space_mapping(t_mins_t_tipOfSignal, PhotoLocation, BigPicRotStruct)
    ...
    if ~~nnz(LL)        % the case where there are different time values
        x_min_Ttip = -interp1(TtipsBigPic-t_front_crop,x_front_crop-PhotoLocation,-t_mins_t_tipOfSignal,'linear','extrap');
        
    else                % if the whole time vector is the same,   THIS IS A CHEATING BYPASS, NOT ACCURATE AT ALL
        XX = x_front_crop-PhotoLocation;
        if min(t_mins_t_tipOfSignal)>max(TtipsBigPic-t_front_crop)
            x_min_Ttip = XX(end);
        else
            x_min_Ttip = XX(1);
        end
