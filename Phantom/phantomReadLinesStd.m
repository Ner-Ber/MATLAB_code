function [StdLines Included] = phantomReadLinesStd(exp_dir,eventNum,startl,interval,endl,smt,includeFlag,lineNum)
...
for ind=startl:interval:endl
    ...
    if (smt>1)
        ...
        StdLines(floor((ind-(startl))/interval)+1,:)=smooth(phantomStdLinesNoOverload(Imr,Included),smt);
        
    else
        %Ims.Lines(floor((ind-(startl))/interval)+1,:)=mean(Imr,1);
        StdLines(floor((ind-(startl))/interval)+1,:)=phantomStdLinesNoOverload(Imr,Included);
...
function std=phantomStdLinesNoOverload(inIm,included)
