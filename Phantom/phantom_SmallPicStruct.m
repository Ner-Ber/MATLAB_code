function RowOverTimeStruct = phantom_SmallPicStruct(exp_dir, eventNum, varargin)
% RowOverTimeStruct = phantom_SmallPicStruct(exp_dir, eventNum, pre_time,intervalTime,post_time,timeBase,smt,lineNum, FallingPercnt)
%
% this function is similar to phantom_BigPicStruct but meant for reading
% ilya's experiments. 
%
...
[pre_time,intervalTime,post_time,timeBase,smt,lineNum, FallingPercnt] = setDefaults4function(varargin,...
    0.5e-3,1e-8,4e-3,1,0,'all',5);
%% get general info
exp_details = expDetailsRead(exp_dir);
CamMetaStructBig = CameraMetaAllCams(exp_dir);
...
%--- normalized ROT:
NormalizedRowOverTime = bsxfun(@rdivide,RowOverTime,RowOverTime(1,:));
