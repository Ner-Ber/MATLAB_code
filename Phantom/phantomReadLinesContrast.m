function [ConLines Included] = phantomReadLinesStd(exp_dir,eventNum,startl,interval,endl,smt,includeFlag,lineNum)
...
for ind=startl:interval:endl
    ...
    Imr=Imr(lineNum,:);
    Imr=Imr./firstImr;
    %Imr=my_smooth(Imr',21)'; %my_smooth is coloumn oriented. I want to smooth with row orientetion
    
    if (smt>1)
        %Ims.Lines(floor((ind-(startl))/interval)+1,:)=smooth(mean(Imr,1),smt);
        ConLines(floor((ind-(startl))/interval)+1,:)=smooth(phantomContrastLinesNoOverload(Imr,Included),smt);
    else
        %Ims.Lines(floor((ind-(startl))/interval)+1,:)=mean(Imr,1);
        ConLines(floor((ind-(startl))/interval)+1,:)=phantomContrastLinesNoOverload(Imr,Included);
    end
...
function con=phantomContrastLinesNoOverload(inIm,included)

inIm=inIm.*included;
num=sum(included,1);

%calculate first moment^2
Sum=sum(inIm,1);
m1=(Sum./num);
m1(isnan(m1))=0;

con=(max(inIm,[],1)-min(inIm,[],1))./m1;
