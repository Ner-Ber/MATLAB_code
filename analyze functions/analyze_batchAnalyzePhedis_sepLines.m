function PhediStructMultipLines = analyze_batchAnalyzePhedis_sepLines(ExperNum, EvNum, lNum_cell, varargin)
    % produces a struct where each field is a PhediStruct of 1 or more lines,
    % by what's indicated in the input lNum_cell.
    [signalSmooth4PhediTrack, BlurROT] = setDefaults4function(varargin,1,1);
    
    
    PhediStructMultipLines = struct;
    for l = 1:length(lNum_cell)
        lNum = lNum_cell{l};
        PhediStruct_thisLine = phedi_createStructureWAdd(ExperNum,EvNum,...
            'preRowsTime',5e-3,'postRowsTime',5e-3,'prePhediTime',3e-3,'postPhediTime',1.8e-3, 'lineNum', lNum,...
            'signalSmooth4PhediTrack', signalSmooth4PhediTrack, 'BlurROT', BlurROT);
        if ischar(lNum)
            PhediStructMultipLines.(['line_',lNum]) = PhediStruct_thisLine;
        else
            PhediStructMultipLines.(['line_',strrep(num2str(lNum),'  ','_')]) = PhediStruct_thisLine;
        end
    end
